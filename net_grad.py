...
class Net:
    def __init__(self, num_basis=90, ksz=15, burst_length=2, channels_count_factor=1):
        ...
        self.kernel_set_count = 4 #3x3,5x5, gx, gy, 
        self.coeff_set_count = 4 #3x3,
    ...
    def create_basis(self):
        ...
        out = self.conv('k_output_2', out, 3 * self.kernel_set_count * self.num_basis, relu=False)
        out = tf.reshape(
            out, [-1, self.ksz * self.ksz * 3 * self.kernel_set_count, self.num_basis])
    ...
    def combine(self):
        ...
        self.kernels = tf.matmul(
            coeffs,
            self.basis
        )  # (h * w) x (ksz * ksz * 3 * kernel_set_count)
        self.kernels = tf.reshape(
            self.kernels, [-1, imsp[1], imsp[2], self.ksz * self.ksz * 3, self.kernel_set_count])
    ...
    def forward(self, inp):
        ...
        gx = tfu.apply_filtering(inp[:, :, :, :3], self.kernels[..., 2])
        gy = tfu.apply_filtering(inp[:, :, :, :3], self.kernels[..., 3])
        filtered_screen_poisson_r = tfu.screen_poisson(1.0,filtered_ambient[...,0],gx[...,0],gy[...,0],self.IMSZ)
        filtered_screen_poisson_g = tfu.screen_poisson(1.0,filtered_ambient[...,1],gx[...,1],gy[...,1],self.IMSZ)
        filtered_screen_poisson_b = tfu.screen_poisson(1.0,filtered_ambient[...,2],gx[...,2],gy[...,2],self.IMSZ)
        filtered_screen_poisson = tf.stack([filtered_screen_poisson_r,filtered_screen_poisson_g,filtered_screen_poisson_b],axis=-1)
        filtered_screen_poisson = tf.reshape(filtered_screen_poisson,[-1,filtered_screen_poisson_r.shape[-2],filtered_screen_poisson_r.shape[-1],3])
        ...
        # filtered_ambient = filtered_screen_poisson + smoothed_ambient
