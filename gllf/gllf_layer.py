...
class gllf_layer_radial(tiny_unet):
    ...
    def scalar_alphas_net(self, bottleneck):
        ...
        if(self.llf_remap_function == 'exp_1d'):
            ...
            range_basis_weights     = tf.reduce_mean(range_basis_weights,axis=(1,2))
            ...
            for i in range(self.max_levels):
                ...
                if(i != self.max_levels - 1):
                    spatial_weights, _  = self.down_block(spatial_weights,  self.channel_count(512//(2**i)), pfx + 'range_basis_down%i' % i)
    ...
    def remapping_1d(self, im_is, im_idx, k):
        ...
        if(self.llf_remap_function == 'exp_1d'):
            i, alpha, beta, sigma = im_is[im_idx], self.alphas[im_idx], self.betas[im_idx], self.sigmas[im_idx]
            ...
            sigma_i = tf.cast(sigma_i, tf.float32)
            i = im_is[im_idx]
    ...
    def visualize(self, im_is):
        ...
        visualizations = []
        reconstructed_images = self.gllf_diffable_1d(im_is, debug_reconstruct_all_remapping_images=True)
        for j in range(self.img_ct):
            ...
            visualizations += [edict(image=reconstructed_images[j], label='$image=%i, reconstructed$'% (j), key='reconstructed_%i' % (j))]
    ...
    def diffable_slice_separable(self, l_i, lpyramids, image_weights):
        ...
        interim_pyramid = tf.transpose(outLPyramids,(0,1,4,2,3))
        ...
        image_weights_interim = tf.transpose(image_weights,(0,3,4,1,2))
        ...
        interim_blocks = tf.transpose(interim_blocks, (0,1,3,5,4,6,2))
    ...
    def gllf_diffable_1d(self, im_is, debug_reconstruct_all_remapping_images=False):
        ...
        if(debug_reconstruct_all_remapping_images):
            for i in range(self.max_levels):
                ...
                outLPyramid_slice = self.diffable_slice_separable_expansion(G_is[i], outLPyramids[i])
                outLPyramid.append(outLPyramid_slice)
            intensity_images = []
            for i in range(self.img_ct):
                img_levels = []
                for j in range(self.max_levels):
                    img_levels.append(outLPyramid[j][:,i])
                intensity_images.append(reconstruct_Laplacian(img_levels, self.max_levels))
            # for i_idx in range(self.max_discrete_levels):
            #     pyramid = []
            #     for l_idx in range(self.max_levels):
            #         pyramid.append(outLPyramids[l_idx][0,:,i_idx,...])
            #     intensity_images.append(reconstruct_Laplacian(pyramid, self.max_levels))
