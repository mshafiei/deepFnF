...
def get_official_model_weights():
    ...
    for k, v in pytorch_model.model.net.net[0].named_parameters():
        vgg_model_weights[k] = v.data

    model_path = './weights/v0.1/vgg.pth'
...
def set_pytorch_pretrained_weights(pytorch_weights, dst_model, correspondence):
    ...
    for src_name, dst_name in correspondence.items():
        ...
        for dst_weight in dst_model.weights:
            if dst_name in dst_weight.path:
...
def export_official_model(image_size, save_dir):
    ...
    # save tf.keras model
    vgg_ckpt_fn = os.path.join(save_dir, 'vgg')
    lin_ckpt_fn = os.path.join(save_dir, 'lin')
    if(not os.path.exists(vgg_ckpt_fn)):
        os.makedirs(vgg_ckpt_fn)
    if(not os.path.exists(lin_ckpt_fn)):
        os.makedirs(lin_ckpt_fn)

    vgg_model.save_weights(os.path.join(vgg_ckpt_fn, 'exported.weights.h5'))
    lin_model.save_weights(os.path.join(lin_ckpt_fn, 'exported.weights.h5'))
    return
...
def run_lpips_metric(image_size, save_dir):
    ...
    vgg_ckpt_fn = os.path.join(save_dir, 'vgg', 'exported.weights.h5')
    lin_ckpt_fn = os.path.join(save_dir, 'lin', 'exported.weights.h5')
    ...
    print('Distance: %.03f' % dist01)
    return


def main():
    # parameters
    ckpt_dir = './weights/keras'
    image_size = 64
