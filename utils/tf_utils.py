...
def screen_poisson(lambda_d, img,grad_x,grad_y,IMSZ):
    ...
    recon_freq = (tf.dtypes.complex(lambda_d,0.) * img_freq + tf.math.conj(Dx_freq) * grad_x_freq + tf.math.conj(Dy_freq) * grad_y_freq) / \
                (tf.dtypes.complex(lambda_d,0.) + (tf.math.conj(Dx_freq) * Dx_freq + tf.math.conj(Dy_freq) * Dy_freq))
