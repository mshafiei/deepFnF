...
from gllf_color_local_alpha_grad_sparse_Mullapudi2016 import gllf_color_local_alpha_grad_sparse_Mullapudi2016 as guided_local_laplacian_grad_Mullapudi2016
...
def denoising(guide, input, levels, alpha, beta, output_path):
    ...
    eps = 0.01
    guided_local_laplacian_grad_Mullapudi2016(guided_buf, input_buf, levels, alpha_map / (levels - 1), beta, aw, ah, w, h, offset_x, offset_y, output_buf)
    ...
    for i in range(ah):
        for j in range(aw):
            ...
            if(dump_small_image):
                ...
                target_startx = 0 if j == 0 else j * ratio_x - offset_x
                target_endx = w if j == aw-1 else (j+1) * ratio_x + offset_x
                target_starty = 0 if i == 0 else i * ratio_y - offset_y
                target_endy = h if i == ah-1 else (i+1) * ratio_y + offset_y
