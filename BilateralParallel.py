...
# local imports
import utils_bilateral as utils
...
reference =     '/home/mohammad/Projects/deepFnF/test_imgs/reference.png'
target =        '/home/mohammad/Projects/deepFnF/test_imgs/target.png'
confidence =    '/home/mohammad/Projects/deepFnF/test_imgs/confidence.png'

def prepare_flow(reference, flow_tuple, confidence, params):
    BILATERAL_SIGMA_SPATIAL = params['BILATERAL_SIGMA_SPATIAL']
    BILATERAL_SIGMA_LUMA = params['BILATERAL_SIGMA_LUMA']
    LOSS_SMOOTH_MULT = params['LOSS_SMOOTH_MULT']
    A_VERT_DIAG_MIN = params['A_VERT_DIAG_MIN']
    NUM_PCG_ITERS = params['NUM_PCG_ITERS']
...
def prepare_input(reference, target, confidence, params):
    BILATERAL_SIGMA_SPATIAL = params['BILATERAL_SIGMA_SPATIAL']
    BILATERAL_SIGMA_LUMA = params['BILATERAL_SIGMA_LUMA']
    LOSS_SMOOTH_MULT = params['LOSS_SMOOTH_MULT']
    A_VERT_DIAG_MIN = params['A_VERT_DIAG_MIN']
    NUM_PCG_ITERS = params['NUM_PCG_ITERS']
    NUM_NEIGHBORS = params['NUM_NEIGHBORS']
    reference_image = np.array(reference, dtype=np.float32)*256
    target_image = np.array(target, dtype=np.float32)

    weight = np.array(confidence, dtype=np.float32)*65536
    weight = np.divide(weight, 65536)
    ...
    X = np.concatenate((I_x[:,:,None],I_y[:,:,None],I_luma[:,:,None]),axis=2).reshape((-1,3),order='F')
    W0 = np.ravel(weight.T)
    X0 = np.reshape(target_image,[-1,1],order='F')
    return X, W0, X0, target_image.shape
...
def make_neighbors(grid_size):
    ...
    for diff in [-1,1]:
        ...
        idx_2ravel = np.add(idx2ravel, diff)
        ...
        idx2ravel = np.array([np.ravel_multi_index(item,dims=grid_size,order='F') for item in idx])
        idx_2ravel = np.add(idx2ravel, grid_size[0] * diff)
        ...
        idx2ravel = np.array([np.ravel_multi_index(item,dims=grid_size,order='F') for item in idx])
        idx_2ravel = np.add(idx2ravel, grid_size[0]*grid_size[1] * diff)

        idxs = np.vstack( [idxs, np.array([idx2ravel,idx_2ravel]).T ] )
    return idxs


def bistochastize(splat_mat, grid_size, diffuse3_mat, W0, Xshape, params):
    BILATERAL_SIGMA_SPATIAL = params['BILATERAL_SIGMA_SPATIAL']
    BILATERAL_SIGMA_LUMA = params['BILATERAL_SIGMA_LUMA']
    LOSS_SMOOTH_MULT = params['LOSS_SMOOTH_MULT']
    A_VERT_DIAG_MIN = params['A_VERT_DIAG_MIN']
    ...
    for i_norm in range(20):
        diffuse_splat_norm = np.reshape(diffuse3_mat * np.reshape(splat_norm, (-1, 1),order='F'), splat_norm.shape,order='F')
...
def dense_solve_jacobi(X, W0, X0, params):
    BILATERAL_SIGMA_SPATIAL = params['BILATERAL_SIGMA_SPATIAL']
    BILATERAL_SIGMA_LUMA = params['BILATERAL_SIGMA_LUMA']
    LOSS_SMOOTH_MULT = params['LOSS_SMOOTH_MULT']
    A_VERT_DIAG_MIN = params['A_VERT_DIAG_MIN']
    ...
    ij = np.clip(ij,a_min=0,a_max=None)
    ...
    splat_norm, A_diag = bistochastize(splat_mat, grid_size, diffuse3_mat, W0, X.shape[0], params)
    if VERBOSE:
    ...
    for iter in range(NUM_PCG_ITERS):
        ...
        Ysn_reshaped = np.reshape(Ysn,(-1,1),order='F')
...
def bilateral_rgb(reference,target, confidence, params):

    input_X, input_W0, input_X0, target_shape = prepare_input(np.clip(reference,0,1),np.clip(target[:,:,0],0,1), confidence[:,:,0],params)
    res_cpu_r = dense_solve_jacobi(input_X, input_W0, input_X0,params)
    res_cpu_r = res_cpu_r.reshape(target_shape[0], target_shape[1], -1,order='F')
    
    input_X, input_W0, input_X0, target_shape = prepare_input(reference,target[:,:,1], confidence[:,:,1],params)
    res_cpu_g = dense_solve_jacobi(input_X, input_W0, input_X0,params)
    res_cpu_g = res_cpu_g.reshape(target_shape[0], target_shape[1], -1,order='F')

    input_X, input_W0, input_X0, target_shape = prepare_input(reference,target[:,:,2], confidence[:,:,2],params)
    res_cpu_b = dense_solve_jacobi(input_X, input_W0, input_X0,params)
    res_cpu_b = res_cpu_b.reshape(target_shape[0], target_shape[1], -1,order='F')

    return np.stack([res_cpu_r,res_cpu_g,res_cpu_b],axis=-1)

def main():
    reference_image = np.array(plt.imread(reference, format='png'), dtype=np.float32)
    target_image = np.array(plt.imread(target, format='png'), dtype=np.float32)
    weight_image = np.array(plt.imread(confidence, format='png'), dtype=np.float32) * 0+1
    tmp = bilateral_rgb(reference_image,np.stack([target_image,target_image,target_image],axis=-1), weight_image)

    input_X, input_W0, input_X0, target_shape = prepare_input(reference_image,target_image, weight_image)

    res_cpu = dense_solve_jacobi(input_X, input_W0, input_X0)
    res_cpu = res_cpu.reshape(target_shape[0], target_shape[1], -1,order='F')

    ref = np.array(plt.imread(reference, format='png'), dtype=np.float32)
    flow = np.array(plt.imread(target, format='png'), dtype=np.float32)
    conf = np.array(plt.imread(confidence, format='png'), dtype=np.float32)
