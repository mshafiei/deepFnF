...
def bilateralFilter(target, reference, params):
    grid_params = {
        'sigma_luma' : params['BILATERAL_SIGMA_LUMA'], # Brightness bandwidt1h
        'sigma_chroma': params['BILATERAL_SIGMA_LUMA'], # Color bandwidth
        'sigma_spatial': params['BILATERAL_SIGMA_SPATIAL'] # Spatial bandwidth
    }
    im_shape = reference.shape[:2]
    grid = BilateralGrid(reference, **grid_params)
    if(len(target.shape) == 2):
        t = target.reshape(-1, 1)
        tc_filt = grid.filter(t * 1)
        return (tc_filt / 1).reshape(im_shape)
    else:
        tc_filt0 = grid.filter(target[:,:,0].reshape(-1, 1) * 1).reshape(im_shape)
        tc_filt1 = grid.filter(target[:,:,1].reshape(-1, 1) * 1).reshape(im_shape)
        tc_filt2 = grid.filter(target[:,:,2].reshape(-1, 1) * 1).reshape(im_shape)
        return np.stack([tc_filt0, tc_filt1, tc_filt2], axis=-1)

def bilateralSolve(target, reference, params):
    grid_params = {
        'sigma_luma' : params['BILATERAL_SIGMA_LUMA'], # Brightness bandwidt1h
        'sigma_chroma': params['BILATERAL_SIGMA_LUMA'], # Color bandwidth
        'sigma_spatial': params['BILATERAL_SIGMA_SPATIAL'] # Spatial bandwidth
    ...
    bs_params = {
        'lam': params['bs_lam'], # The strength of the smoothness parameter
    ...
    if(len(target.shape) == 2):
        return BilateralSolver(grid, bs_params).solve(target.reshape(-1, 1), np.ones_like(target.reshape(-1, 1))).reshape(im_shape)
    else:
        tc_filt0 = BilateralSolver(grid, bs_params).solve(target[:,:,0].reshape(-1, 1), np.ones_like(target[:,:,0].reshape(-1, 1))).reshape(im_shape)
        tc_filt1 = BilateralSolver(grid, bs_params).solve(target[:,:,1].reshape(-1, 1), np.ones_like(target[:,:,1].reshape(-1, 1))).reshape(im_shape)
        tc_filt2 = BilateralSolver(grid, bs_params).solve(target[:,:,2].reshape(-1, 1), np.ones_like(target[:,:,2].reshape(-1, 1))).reshape(im_shape)
        # tc_filt0 = grid.filter(target[:,:,0].reshape(-1, 1) * 1).reshape(im_shape)
        # tc_filt1 = grid.filter(target[:,:,1].reshape(-1, 1) * 1).reshape(im_shape)
        # tc_filt2 = grid.filter(target[:,:,2].reshape(-1, 1) * 1).reshape(im_shape)
        return np.stack([tc_filt0, tc_filt1, tc_filt2], axis=-1)
