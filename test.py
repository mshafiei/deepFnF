...
def test_single_image(data, logger, model, errval, metrics, metrics_list, errors_dict, levelKey, iteration_count, filename):
    ...
    if(iteration_count % logger.opts.visualize_freq_test == 0):
        ...
        name=os.path.basename(filename) + ('_%s' % (levelKey))
        logger.addImage(images, lbls, name, cols=6, annotation={'output':annotation}, mode='test', image_filename=filename, font_size_scale=2,vertical_spacing_scale=2, subtext=subtext)
        logger.addIndividualImages({name:tf.clip_by_value(images['output'],0,1)}, {name:lbls['output']})
